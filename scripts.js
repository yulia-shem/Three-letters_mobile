...
let words = ["дом", "кот", "пар", "лес", "жук", "куб", "шар", "душ", "мир", "мак", "рок", "нос", "сыр", "мул", "миг", "фен", "нюх", "сев", "лис", "тис", "бок", "бар", "бра", "гад", "ухо", "вид", "акт", "акр", "имя", "боб", "икс", "воз", "зов", "яма", "мяч", "эхо", "юла", "ель", "аул", "щуп", "гол", "гул", "вол", "вал", "дар", "дым", "зло", "чип", "фут", "ярд", "йод"];
let answer;
...
function makeRowInteract(rowIndex) {
    ...
    for (let i = 0; i < row_inputs.length - 1; i++) {
        row_inputs[i].addEventListener("input", () => {
            // перебор всех, кроме последней
            // console.log(e);
            if (alphabet.includes(row_inputs[i].value)) {
    ...
    row_inputs[row_inputs.length - 1].addEventListener("input", () => {
        if (alphabet.includes(row_inputs[row_inputs.length - 1].value)) {
